...
# load(file=file.path(figsPath, "prevalence_swe.RData"))

# image
# save.image(file=file.path(figsPath, "prevalence_swe.RData"))
...
phylum_table_swe <- phyloseq_summarize_taxa(swe_physeq_clean, 'phylum')

sort(taxa_sums(phylum_table_swe)/sum(taxa_sums(phylum_table_swe)))

# get overall reads per genus
genus_table_swe <- phyloseq_summarize_taxa(swe_physeq_clean, 'genus')

sort(taxa_sums(genus_table_swe)/sum(taxa_sums(genus_table_swe)))
...
taxa = get_taxa_unique(swe_physeq_clean, "phylum")

ntaxa(swe_physeq_clean)

tdt = data.table(tax_table(swe_physeq_clean),
                 TotalCounts = taxa_sums(swe_physeq_clean),
                 OTU = taxa_names(swe_physeq_clean))
...
png(file.path(figsPath, "pCumSum_swe.png"), width=86, height=86, units="mm", pointsize = 4, bg = FALSE, res = 300)
...
# Prevalence
mdt = fast_melt(swe_physeq_clean)
...
tax_table(prune_taxa(over10$TaxaID, swe_physeq_clean))
...
png(file.path(figsPath, "hist_prev_swe.png"), width=86, height=86, units="mm", pointsize = 4, bg = FALSE, res = 300)
  plot_prev
dev.off()
...
png(file.path(figsPath, "plot_prev_swe.png"), width=86, height=86, units="mm", pointsize = 4, bg = FALSE, res = 300)
  plot_prevcumsum
dev.off()
...
png(file.path(figsPath, "plot_prev_totcounts_swe.png"), width=86, height=86, units="mm", pointsize = 2, bg = FALSE, res = 300)
  plot_prev_totcounts
dev.off()
...
total_reads = sum(colSums(otu_table(swe_physeq_clean)))
...
png(file.path(figsPath, "plot_relcounts_relprev_swe.png"), width=86, height=86, units="mm", pointsize = 2, bg = FALSE, res = 300)
  plot_relcounts_relprev
dev.off()

## Lakes
# 
asv_table <- as.data.frame(t(otu_table(swe_physeq_clean)))
