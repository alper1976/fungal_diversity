...
option_list = list(
    ...
    make_option(c("-p", "--path"), 
        type="character",
        default="/cluster/projects/nn9745k/02_results/02_hundred_lakes/fungi/AdaptersRemoved",   #path to adapters_removed
        help="path to input",
        metavar="character"),

    make_option(c("-o", "--output"),
        type="character",
        default="/cluster/projects/nn9745k/02_results/02_hundred_lakes/fungi",
        help="path to output",
        metavar="character"),
...
FWD <-"AACCAWCGATGAAGAACGCAG"    #INSERT_FWD_PRIMER
REV <- "AATCCTCCGCTTAYTRATATGC"    #INSERT_REV_PRIMER
...
setwd("/cluster/projects/nn9745k/02_results/02_hundred_lakes/fungi")
path <- getwd()
...
# load the seqtab_nochim
seqtab_nochim = readRDS("seqtab_nochim.rds")


# Edit/clean sample names  ########### keeping just the beginning of the names (before the first "_") 
seqtab_names = str_extract(rownames(seqtab_nochim), "([^_]+)")
    ...
    sinkname <- file.path(out_path, paste0(rownames(my_table)[sampleX],".fas"))
    {
      ...
      for (seqX in seq(1:dim(my_table)[2])) {
        if (my_table[sampleX,seqX] > 0) {
          header <- paste0(">",rownames(my_table)[sampleX],"_Seq",seqX,";size=",my_table[sampleX,seqX],"\n")   ##### Edited later to get read names like "L34_Seq345; size=3454"; initially (without "SeqX") I found duplicated read IDs!!  
          cat(header)
...
#### get and save read length  (I could not manage it in my PC)
